...
import { OCI_BV_DRIVER, OCI_FS_DRIVER } from '@shell/models/persistentvolume';
...
export default {
  ...
  data() {
    const driverOptions = [
      {
        label: this.t('persistentVolume.oci.bv'),
        value: OCI_BV_DRIVER
      },
      {
        label: this.t('persistentVolume.oci.fs'),
        value: OCI_FS_DRIVER
      }
    ];
    ...
    if (this.mode === _CREATE) {
      ...
      this.$set(this.value.spec.csi, 'driver', OCI_BV_DRIVER);
    ...
    return { readOnlyOptions, driverOptions };
  },
};
</script>
    ...
    <div class="row mb-20">
      <div class="col span-6">
        <RadioGroup
          ...
          name="driver"
          :mode="mode"
          :label="t('persistentVolume.oci.driver')"
